...
Der quelloffene Open-Access-Metadaten-Schnelltest erlaubt die umfangreiche Überprüfung von bis zu `r formals(metacheck::is_in_limit)$limit` DOIs pro Bericht und liefert ein Untersuchungsergebnis innerhalb von 45 Minuten per Email. 
...
Gültige DOIs müssen folgende Kriterien erfüllen:

```{r pretest_criteria, echo=FALSE}
library(metacheck)
transl <- mc_translator()
transl$set_translation_language("de")
knitr::kable(
  tibble::tibble(
    Name = pretests()$name,
...
Weitere Details in `metacheck::pretests()`.
