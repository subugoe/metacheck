...
You can check up to `r formals(metacheck::is_in_limit)$limit` DOIs at once using this freely available tool. 
...
Eligible DOIs must meet the following criteria.

```{r pretest_criteria, echo=FALSE}
library(metacheck)
transl <- mc_translator()
transl$set_translation_language("en")
knitr::kable(
  tibble::tibble(
    Name = pretests()$name,
    Beschreibung = transl$translate(pretests()$desc)
  )
)
```

See `metacheck::pretests()` for more details.
